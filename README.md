...
**当前版本：v1.9.46（Cloudflare Temp Email · 续期/首包/空响应加固）**
...
```bash
docker pull ghcr.io/hm2899/grokcli-2api:1.9.46
# 或
...
services:
  ...
  grokcli-2api:
    image: ghcr.io/hm2899/grokcli-2api:1.9.46
    ports:
...
- `ghcr.io/hm2899/grokcli-2api:1.9.46`
...
> 镜像 tag 与 `app.py` 中 `APP_VERSION` 一致（当前 **1.9.46**）。推 `main` 会打 `edge` 与版本号；打 `v1.9.46` tag 会额外发布 `latest`。  
